...
const QuranChapter = () => {
  ...
  const playWholeSurah = () => {
    axios
      .get("/v1/recitation/chapter/" + chapter_number + "/by/7")
      .then(({ data }) => {
        console.log(data);
        setCurrentAudio(data.audio_file.audio_url);
        audio.audioEl.current.play();
        setShowControl(true);
      });
  };
  ...
  return (
    <div className="container px-5 text-white font-arabic">
      <div className="flex justify-between items-center">
        <h1 className="py-10 font-semibold">{chapter?.name_simple}</h1>
        ...
        {verses?.map((verse, i) => (
          <div key={i} className="flex gap-10 border-b border-slate-500 py-5">
            ...
            <div className="flex-grow text-xl">
              ...
              <div className="pt-8">{verse.translations[0].text}</div>
            </div>
          </div>
        ))}
      </div>
